...
bool AudioEngine::checkStepTick() {
  if (n_samples_since_last_step_tick <
      n_samples_per_step_tick_ + fractional_sample_offset) {
  ...
  fractional_sample_remainder +=
      n_samples_per_step_tick_ - SDL_trunc(n_samples_per_step_tick_);
...
void audioCallback(void *userdata, SDL_AudioStream *stream,
  ...
  while (additional_amount > 0) {
    ...
    // cannot use ranged for loop because of possible vector reallocation
    {
      ...
      SDL_GetAudioStreamData(gen_block->getStream(), new_samples,
  ...
  for (int i = 0; i < WAVE_SIZE; i++) {
    ...
    for (int harmonic = 1; harmonic <= MAX_HARMONIC; harmonic++) {
      ...
      wavetables_.at({WAVE_SAW, harmonic}).at(i) =
          prev - (2.0 * INV_PI) * ((-1) + 2 * (harmonic % 2)) *
                     sinf(harmonic * phase) / harmonic;

      // TRIANGLE
      prev = wavetables_.at({WAVE_TRIANGLE, harmonic - 1}).at(i);
      wavetables_.at({WAVE_TRIANGLE, harmonic}).at(i) =
          prev + -8.0 * INV_PI_SQ * ((-1) + 2 * (harmonic % 2)) *
                     sinf((2 * harmonic - 1) * phase) /
    ...
    for (int harmonic = 1; harmonic <= MAX_HARMONIC; harmonic++) {
      ...
      wavetables_.at({WAVE_SQUARE, harmonic}).at(i) =
          prev +
          4.0 * INV_PI * sinf((2 * harmonic - 1) * phase) / (2 * harmonic - 1);
    }
  }
}

std::array<float, WAVE_SIZE> *AudioEngine::getWaveTable(WAVE_FORMS form,
                                                        pitch_t freq) {
...
std::map<std::pair<WAVE_FORMS, int>, std::array<float, WAVE_SIZE>>
    AudioEngine::wavetables_ = {};
