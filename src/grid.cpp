...
void Grid::draw(SDL_Renderer *renderer) {
  ...
  for (int i = 0; i < GRID_SIZE; ++i) {
    ...
    SDL_RenderLine(renderer, Camera::worldToScreen(hori_from).x,
    ...
    SDL_RenderLine(renderer, Camera::worldToScreen(vert_from).x,
...
void drawBlockText(Block *block, std::string type, std::string value,
                   SDL_Color color) {
  ...
  Vector2f value_pos = {rect->x + rect->w / 2, rect->y + rect->h / 2};
...
void drawGenerator(SDL_Renderer *renderer, BlockGenerator *block) {
  SDL_SetRenderDrawColor(renderer, GENERATOR_COLOR.r, GENERATOR_COLOR.g,
                         GENERATOR_COLOR.b, GENERATOR_COLOR.a);
  ...
  if (block->isInArea())
    type_text = std::format("{:.2f} * (x + {:.2f})Hz", block->getFreqFactor(),
                            block->getRelFreq());
  else
...
void drawSequencer(SDL_Renderer *renderer, BlockSequencer *block) {
  ...
  SDL_SetRenderDrawColor(renderer, SEQUENCER_COLOR.r, SEQUENCER_COLOR.g,
                         SEQUENCER_COLOR.b, SEQUENCER_COLOR.a);
  SDL_RenderFillRect(renderer, block->getFRect());
  ...
  case PITCH_INTERVAL: {
    ...
    value_text =
        std::format("{:.1f} / {:.1f}", interval.first, interval.second);
    break;
  }
  case PITCH_NOTE: {
    type_text = "note";
...
void Grid::drawAreas(SDL_Renderer *renderer) {
  SDL_SetRenderDrawColor(renderer, AREA_COLOR.r, AREA_COLOR.g, AREA_COLOR.b,
                         AREA_COLOR.a);
  SDL_Rect render_rect;
  SDL_FRect render_frect;
  SDL_GetRenderViewport(renderer, &render_rect);
  ...
  for (auto &area : areas_) {
    for (auto &pos : area->getPositions()) {
      SDL_FRect rect = {Camera::worldToScreen(pos).x,
                        Camera::worldToScreen(pos).y, Camera::getZoom(),
                        Camera::getZoom()};
...
void Grid::clear() {
  for (int a = areas_.size() - 1; a >= 0;
       a--) // cannot use ranged for because area vector gets reallocated
...
std::vector<Block *> Grid::getBlocks() {
  ...
  for (auto &area : areas_) {
    ...
    blocks.insert(std::end(blocks), std::begin(area_blocks),
                  std::end(area_blocks));
...
std::array<Area *, 4>
Grid::getAdjacentAreas(Vector2f pos) // in order UP, LEFT, DOWN, RIGHT
{
  pos = floorVec(pos);
...
Area *Grid::connectAreas(Vector2f pos) {
  ...
  int first_neighbour;
  // sets first_neighbour to the first index of the adjacent array that isn't
  // nullptr: UP, LEFT, DOWN, RIGHT
  ...
  if (area) {
    ...
    // merges all consecutive neighbours with the first previously found
    // neighbour
...
void Grid::splitAreas(Area *area) {
  ...
  while (!remaining_pos.empty()) {
    ...
    while (!visit_queue.empty()) {
      ...
      if (std::find(connected.begin(), connected.end(), current_pos) !=
          connected.end())
        continue;
    ...
    for (auto &pos : connected) {
      remaining_pos.erase(
          std::remove(remaining_pos.begin(), remaining_pos.end(), pos),
          remaining_pos.end());
      Block *block = area->getBlock(pos);
      if (block) {
        area->removeBlock(block);
    ...
    new_area->setStepCounter(area->getStepCounter());
...
void Grid::pasteBlock(Vector2f pos) {
  if (pos.x >= GRID_SIZE / 2 || pos.y >= GRID_SIZE / 2 ||
      pos.x < -GRID_SIZE / 2 || pos.y < -GRID_SIZE / 2)
    return;
