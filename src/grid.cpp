...
void drawGenerator (SDL_Renderer* renderer, BlockGenerator* block)
{
    ...
    if (block->isInArea())
        type_text =
            std::format ("{:.2f} * (x + {:.2f})Hz", block->getFreqFactor(), block->getRelFreq());
    else
...
std::vector<Block*> Grid::getBlocks()
{
    ...
    for (auto& area : areas_)
    {
        ...
        // remove nullptrs
        sequence.erase (std::remove_if (sequence.begin(),
                                        sequence.end(),
...
void Grid::splitAreas (Area* area)
{
    ...
    while (! remaining_pos.empty())
    {
        ...
        {
            remaining_pos.erase (std::remove (remaining_pos.begin(), remaining_pos.end(), pos),
                                 remaining_pos.end());
            Block* block = area->getBlock (pos);
            if (block)
            {
...
{
    areas_.erase (
        std::remove_if (areas_.begin(), areas_.end(), [area] (auto a) { return a == area; }),
        areas_.end());
...
{
    if (pos.x >= (float) GRID_SIZE / 2 || pos.y >= (float) GRID_SIZE / 2
        || pos.x < -(float) GRID_SIZE / 2 || pos.y < -(float) GRID_SIZE / 2)
        return;
