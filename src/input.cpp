...
bool Input::isKeyDown(SDL_Keycode key) {
  return std::find(keys_down_.begin(), keys_down_.end(), key) !=
         keys_down_.end();
}
...
void Input::handleKeys(SDL_Event *event) {
  ...
  // handleUndoRedo(event); //doesn't work for block parameter changes, no time
  // to add :)
}

void Input::handleLoseFocus() { keys_down_.clear(); }
...
void Input::handleShortcuts(SDL_Event *event) {
  ...
  if ((key.key == SDLK_Q || key.key == SDLK_ESCAPE) &&
