...
Interface::Interface() : camera_{Camera::getInstance()} {
  ...
  if (!SDL_CreateWindowAndRenderer(TITLE, width_, height_, 0, &window_,
                                   &renderer_))
    exit(SDL_APP_FAILURE);
...
void Interface::togglePlayPause() {
  ...
  AudioEngine::setRunning(is_playing_);
...
void Interface::addBlock(Block *block) { grid_.addBlock(block); }

bool Interface::removeBlock(Vector2f pos) { return grid_.removeBlock(pos); }

void Interface::removeBlock(Block *block) { grid_.removeBlock(block); }
...
void Interface::drawGrid() { grid_.draw(renderer_); }

void Interface::drawAreas() { grid_.drawAreas(renderer_); }

void Interface::drawBlocks() { grid_.drawBlocks(renderer_); }
...
void Interface::drawSequencerGUI() {
  ...
  // remove nullptrs
  sequencers.erase(std::remove_if(sequencers.begin(), sequencers.end(),
  ...
  sequencers.erase(std::unique(sequencers.begin(), sequencers.end()),
                   sequencers.end());
...
void Interface::drawBlockSelection() {
  SDL_FRect rect = {ICON_SIZE * RENDER_SCALE / 8, ICON_SIZE * RENDER_SCALE / 8,
