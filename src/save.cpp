...
void save_dialog_callback(void *userdata, const char *const *filelist,
                          int filter);
void load_dialog_callback(void *userdata, const char *const *filelist,
                          int filter);
...
void SaveLoad::save(bool save_as) {
  ...
  if (save_as || save_path.empty()) {
    ...
    SDL_ShowSaveFileDialog(save_dialog_callback, (void *)grid, window,
                           &save_filter, 1, work_dir);
  } else {
...
void SaveLoad::load() {
  ...
  SDL_ShowOpenFileDialog(load_dialog_callback, (void *)grid, window,
                         &save_filter, 1, work_dir, false);
#endif
}
...
void SaveLoad::saveJSON(std::string path, Grid *grid) {
  ...
  if (path.length() < file_ext.length() ||
      path.substr(path.length() - file_ext.length()) != file_ext) {
    path += "." + file_ext;
  }
...
void SaveLoad::loadJSON(std::string path, Grid *grid) {
  ...
  for (auto &sequencer_json : j["sequencers"]) {
    ...
    sequencer->setInterval(sequencer_json["interval"],
                           sequencer_json["octave_subdivision"]);
