...
SDL_Texture *Text::getTexture(std::string text, SDL_Color col) {
  SDL_Surface *surface =
      TTF_RenderText_Blended(default_font_, text.c_str(), 0, col);
...
void Text::drawTexture(SDL_Texture *texture, Vector2f pos, Vector2f constraints,
                       bool centered) {
...
Vector2f Text::getTextSize(std::string text) {
  int width, height;
  TTF_Text *ttf_text =
      TTF_CreateText(text_engine_, default_font_, text.c_str(), 0);
